...
class TaBERTTuner(pl.LightningModule):
    def __init__(self, hparams):
        ...
        #attention: concat column_encoding and context_encoding first, and then apply to them together
        #weighted sum of their concat
        
        #weight: (bs, m, 768)->(bs, m, 768)->(bs, m, 1)->(bs, m), weight*encoding: (bs,1,m)*(bs,m,768)->(bs,1,768)
    ...
    def forward(self, context_list, table_list):
        ...
        #concat column_encoding and context_encoding
        concat = torch.cat([context_encoding, column_encoding], dim=1) #(bs,m,768)
        
        #print('\nConcat size: {}\n'.format(concat.size()))
        
        #apply attention to concat
        out = F.tanh(self.lin_bias(concat)) #(bs, m, 768)

        attn_out = torch.bmm(out,self.att_weight.repeat(bs,1,1)).squeeze(dim=2) #(bs, m)
        
        sm_out2 = self.sm_att(attn_out) #(bs, m)
        
        concat_sum = torch.bmm(sm_out2.unsqueeze(dim=1),concat).squeeze(dim=1) #(bs, 768)
      
        ##print('\nConcat_sum size: {}\n'.format(concat_sum.size()))
        
        out = self.l1(concat_sum) 
        #out = F.relu(out)
        out = F.tanh(out)
        out = self.l2(out)
        out = self.sm(out)
...
if __name__=='__main__':
    ...
    checkpoint_callback = pl.callbacks.ModelCheckpoint(
        filepath=args.output_dir, prefix="attention2_checkpoint", monitor="val_loss", mode="min", save_top_k=1)
